...
export default function RequestCard(appointment) {
  ...
  const options = ["Aceptada", "Rechazada"];
  
  const acceptRequest = (status, appointment) => {
    setRequest({appointment_request:status.option})
    //console.log("Estado:",request);
    setRoute(`http://localhost:3001/appointments/${appointment.appointment.id_picker}/picker/${appointment.appointment.id_appointment}/request`)
